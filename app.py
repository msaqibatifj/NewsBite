...
ALLOW_PROMPT = os.getenv("ALLOW_PROMPT", "false").lower() == "true"
...
def get_api_key(name, env_var, allow_prompt=False):
    key = os.getenv(env_var)
    if key:
        return key

    if allow_prompt and sys.stdin.isatty():
        return getpass.getpass(f"Please enter your {name} API Key: ")

    raise RuntimeError(
        f"Missing {name} API key. Set {env_var} in your environment or .env file."
    )
...
groq_key = get_api_key("Groq", "GROQ_API_KEY", allow_prompt=ALLOW_PROMPT)
...
cleanup_thread = threading.Thread(target=cleanup_expired_audio, daemon=True)
